...
## 15. v2.6.2 — 1차 클릭이 탭을 전환하지 못하던 문제 (2026-08-06)

**가장 심각했던 오작동.** 1차 문자열 '평택' 을 찾아 클릭했다고 로그에는 나오는데
실제로는 탭이 전환되지 않아 **포항 탭 목록에서 2차 탐색이 돌았고, 그래서 무조건 "발견" 팝업이 떴다.**

### 15-1. 원인 — 내가 만든 회귀

v2.3.3 에서 "클릭 동작만 전달되면 성공" 으로 판정을 바꿨다.
결과가 비어 화면이 안 바뀌는 페이지를 실패로 오판하지 않으려던 것이었는데,
그 부작용으로 **ACTION_CLICK 이 아무 일도 안 해도 좌표 탭으로 넘어가지 않게** 되었다.

실기기 로그가 그대로 보여준다.

```
click via ACTION_CLICK fired but screen did not change - trying next method
1차 클릭 성공 [gesture-tap] "평택(만포대)" · 화면 전환 확인
```

**이 사이트에서 ACTION_CLICK 은 true 를 돌려주고도 실제로는 탭을 전환하지 못한다.
좌표 탭(gesture-tap)만 전환시킨다.**

### 15-2. 수정

**(1) 폴백 체인 복구** — 화면이 실제로 바뀐 수단이 나올 때까지 다음 수단으로 넘어간다.
모든 수단을 다 써도 안 바뀌면 `clicked=true, changed=false` 로 돌려주고 무한 재시도는 하지 않는다.

**(2) 탭 전환 실패 시 2차 탐색 금지** — 이게 오탐의 직접 차단이다.

```
탭이 전환되면 목록이 바뀌거나 비워지므로 화면은 반드시 변한다.
변화가 없다 = 여전히 이전 탭(포항) = 이 상태로 2차를 찾으면 엉뚱한 목록에서 발견이 뜬다.
→ 최대 3회 재시도 후, 그래도 안 되면 2차 탐색을 건너뛰고 라운드를 끝낸다.
```

**(3) 성공한 수단 기억** — 직전에 실제로 화면을 바꾼 수단을 다음 클릭에서 먼저 쓴다.
매 라운드 헛치는 시도를 줄인다.

**(4) 클릭 확인 창 최소 4초** — 탭 전환이 느려 2초 안에 안 끝나면 성공한 클릭도
실패로 보고 다음 수단을 또 쏘고 있었다.

**(5) `preferGestureTap` 기본값을 true 로 변경** — ACTION_CLICK 이 웹페이지에서
조용히 무시되는 경우가 많아 좌표 탭을 기본으로 둔다.

### 15-3. 실기기 검증

```
pull-to-refresh #1                                       ← 새로고침 1회
새로고침 확인됨 [pull-to-refresh]
1차 문자열 등장 확인
1차 클릭했으나 화면이 그대로 - 탭 전환 안 됨 [ACTION_CLICK] · 1/3
1차 클릭 성공 [gesture-tap] "평택(만포대)" · 화면 전환 확인   ← 실제 전환
2차 문자열 미발견 - 5.0초 쉬었다가 다음 라운드              ← 오탐 사라짐
```

전환 전에는 포항 목록으로 매번 "발견" 이 떴으나, 이제 평택 탭으로 전환한 뒤
평택 목록에 조건이 없으면 **정상적으로 미발견 처리**된다.

### 15-4. 남은 것

기존에 저장된 설정에는 `preferGestureTap=false` 가 들어 있어 라운드마다
ACTION_CLICK 을 한 번 헛친다(약 4초 손해). 설정 화면에서
`고급 설정 > 1차 클릭에 좌표 탭 우선` 을 켜면 없어진다.
새로 설치하는 경우에는 기본값이 켜짐이라 해당 없음.
